...
class TaskManagerServiceTests {
    ...
    void shouldStartStandardTask() {
        ...
        ReplicateActionResponse actionResponse =
                taskManagerService.start(getTaskDescriptionBuilder(OrderTag.STANDARD).build());
    ...
    void shouldNotStartSinceCannotContributeStatusIsPresent() {
        when(contributionService.getCannotContributeStatusCause(CHAIN_TASK_ID))
                .thenReturn(List.of(new WorkflowError(CONTRIBUTION_TIMEOUT)));
    ...
    void shouldNotDownloadAppSinceCannotContributionStatusIsPresent() {
        ...
        ReplicateActionResponse actionResponse =
                taskManagerService.downloadApp(getTaskDescriptionBuilder(OrderTag.STANDARD).build());
    ...
    void shouldReturnSuccessAndNotDownloadDataSinceEmptyUrls() throws Exception {
        final DealParams dealParams = DealParams.builder()
                ...
                .build();
        final TaskDescription taskDescription = getTaskDescriptionBuilder(OrderTag.STANDARD)
                .datasetUri("")
                .dealParams(dealParams)
                .build();
        when(contributionService.getCannotContributeStatusCause(CHAIN_TASK_ID))
    ...
    void shouldComputeStandardTask() {
        ...
        assertThat(replicateActionResponse)
                .isNotNull()
                .isEqualTo(ReplicateActionResponse.successWithDetails(
                        ReplicateStatusDetails.builder()
                                .exitCode(0)
                                .computeLogs(ComputeLogs.builder()
                                        .stdout("stdout")
                                        .stderr("stderr")
                                        .build())
                                .build()));
    ...
    void shouldComputeTeeTask(final OrderTag orderTag) {
        ...
        assertThat(replicateActionResponse)
                ...
                .isEqualTo(ReplicateActionResponse.successWithDetails(
                        ReplicateStatusDetails.builder()
                                .exitCode(0)
                                .computeLogs(ComputeLogs.builder().stdout("stdout")
                                        .stderr("stderr")
                                        .build())
                                .build()));
    }
    ...
    void shouldNotComputeSinceCannotContributeStatusIsPresent(ReplicateStatusCause replicateStatusCause) {
        when(contributionService.getCannotContributeStatusCause(CHAIN_TASK_ID))
                .thenReturn(List.of(new WorkflowError(replicateStatusCause)));

        final ReplicateActionResponse replicateActionResponse = taskManagerService.compute(getTaskDescriptionBuilder(OrderTag.STANDARD).build());
    ...
    void shouldNotComputeSinceFailedLasStart() {
        ...
        when(computeManagerService.isAppDownloaded(taskDescription.getAppUri()))
                .thenReturn(true);
        when(teeServicesManager.getTeeService(taskDescription.getTeeFramework()))
                .thenReturn(teeMockedService);
        when(teeMockedService.prepareTeeForTask(CHAIN_TASK_ID))
                .thenReturn(false);
    ...
    void shouldNotComputeSinceFailedPreCompute() {
        ...
        when(computeManagerService.isAppDownloaded(taskDescription.getAppUri()))
                .thenReturn(true);
        when(computeManagerService.runPreCompute(taskDescription))
                .thenReturn(PreComputeResponse.builder()
                        .exitCauses(List.of(new WorkflowError(PRE_COMPUTE_DATASET_URL_MISSING)))
                        .build());
        when(computeManagerService.runCompute(taskDescription))
    ...
    void shouldNotContributeAdnFinalizeSinceTrustNotOne() {
        ...
        when(contributionService.getCannotContributeAndFinalizeStatusCause(CHAIN_TASK_ID))
                .thenReturn(List.of(new WorkflowError(TRUST_NOT_1)));
        ReplicateActionResponse replicateActionResponse = taskManagerService.contributeAndFinalize(CHAIN_TASK_ID);
        assertThat(replicateActionResponse)
                .isNotNull()
    ...
    void shouldNotContributeAndFinalizeSinceTaskAlreadyContributed() {
        ...
        when(contributionService.getContribution(computedFile)).thenReturn(contribution);
        when(contributionService.getCannotContributeAndFinalizeStatusCause(CHAIN_TASK_ID))
                .thenReturn(List.of(new WorkflowError(TASK_ALREADY_CONTRIBUTED)));
        ReplicateActionResponse replicateActionResponse = taskManagerService.contributeAndFinalize(CHAIN_TASK_ID);
        assertThat(replicateActionResponse)
                .isNotNull()
