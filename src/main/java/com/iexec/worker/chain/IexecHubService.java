...
public class IexecHubService extends IexecHubAbstractService implements Purgeable {
    ...
    synchronized TransactionReceipt submit(final String function, final String txData) {
        try {
            ...
            log.error("{} thread has been interrupted", function, e);
            Thread.currentThread().interrupt();
        }
    ...
    TransactionReceipt waitTxMined(final String txHash) throws InterruptedException {
        ...
        while (receipt == null && attempt < 3 * blockTime.toSeconds()) {
            TimeUnit.SECONDS.sleep(1L);
            attempt++;
            receipt = web3jService.getTransactionReceipt(txHash);
        }
        if (receipt == null) {
            log.warn("No transaction receipt [hash:{}, attempt:{}]", txHash, attempt);
            failureCounter.increment();
            return null;
        }
        log.info("Transaction receipt [hash:{}, status:{}, revert-reason:{}, attempt:{}]",
                txHash, receipt.getStatus(), receipt.getRevertReason(), attempt);
