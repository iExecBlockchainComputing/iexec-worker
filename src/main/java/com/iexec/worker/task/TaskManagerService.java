    ...
    ReplicateActionResponse compute(TaskDescription taskDescription) {
        ...
        if (!preResponse.isSuccessful()) {
            return getFailureResponseAndPrintError(
                    preResponse.getExitCauses().get(0),
                    context,
                    chainTaskId
            );
        }
        ...
        if (!appResponse.isSuccessful()) {
            final ReplicateStatusCause cause = appResponse.getExitCauses().get(0);
        ...
        if (!postResponse.isSuccessful()) {
            ReplicateStatusCause cause = postResponse.getExitCauses().get(0);
            logError(cause, context, chainTaskId);
